...
load_dotenv(dotenv_path="~/application/ImageCalendar/.env")
...
def upload_image(path: str, bucket: str, key: str):
    s3 = boto3.client('s3',
                      aws_access_key_id="AKIAS3JEUHQ4IUWWBP6B",
                      aws_secret_access_key="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", )
    ...
    for text in to_look:
        if "ParentId" not in text:
            if round(text['Geometry']['BoundingBox']['Height'], 1) == large:
                ...
                blurb.append(text['DetectedText'])
    return res, blurb
